    ...
    def apply(self, job: Job, dry_run: bool = False, screenshot_cb=None) -> bool:
        ...
        try:
            ...
            if apply_btn.count() == 0 or not apply_btn.is_visible():
                text_btn = self.page.locator("button:has-text('Apply'), a:has-text('Apply'), button:has-text('Quick Apply')").first
                if text_btn.count() > 0 and text_btn.is_visible():
                    apply_btn = text_btn
                    logger.info("FounditApply: Using text-based Apply button fallback.")
