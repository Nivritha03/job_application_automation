...
class LeverApply(UniversalApplyEngine):
    ...
    def apply(self, job: Job, dry_run: bool = False, screenshot_cb=None) -> bool:
        ...
        try:
            ...
            logger.info(f"LeverApply: Navigating to apply page: {apply_url}")
            ...
            try:
                ...
                logger.warning("LeverApply: Timed out waiting for form fields — continuing.")
            time.sleep(1)
            ...
            if not dry_run:
                self.submit(submit_selector="button[type='submit'], #btn-submit")
                ...
                if not success:
                    logger.warning("LeverApply: Validation error or captcha detected.")
                    return False
                return True
            else:
                logger.info(f"LeverApply: [DRY RUN] Would submit application for {job.title}")
                return True

        except Exception as e:
            logger.error(f"LeverApply: Application engine failed: {e}")
    ...
    def handle_resume(self, profile_fields, resume_used: str):
        if "resume" in profile_fields and resume_used:
            ...
            if os.path.exists(resume_path):
                ...
                logger.error(f"LeverApply: Resume file not found at {resume_path}")
    ...
    def _detect_success(self) -> bool:
        ...
        if "confirmation" in current_url or "thank-you" in current_url or "success" in current_url or "thanks" in current_url:
            return True
        for selector in ["h1", "h2", "h3", ".confirmation", ".thank-you", ".thanks"]:
            try:
