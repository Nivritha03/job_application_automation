...
class HiristJobParser(BaseJobParser):
    def extract_details(self, job: Job) -> Job:
        logger.info(f"HiristJobParser: Extracting details for {job.url}")
        try:
            try:
                self.page.goto(job.url, wait_until="domcontentloaded", timeout=15000)
            except Exception as e:
                logger.warning(f"HiristJobParser: Page goto warning: {e}")
                
            try:
                self.page.wait_for_selector("h1, div.job-description, .job-details", timeout=15000)
            except Exception as wait_err:
                logger.warning(f"HiristJobParser: Selector wait timeout: {wait_err}")
                
            time.sleep(1)
            
            # Check login redirects
            if "login" in self.page.url or "signin" in self.page.url:
                logger.error("HiristJobParser: Authentication required.")
                job.error_message = "Authentication required to view details."
                return job
                
            # Title
            title_loc = self.page.locator("h1, .job-title, [class*='jobTitle']").first
            ...
            # Description
            desc_loc = self.page.locator(".job-description, div.job-desc, .job-details").first
            if desc_loc.count() > 0:
            ...
            logger.error(f"HiristJobParser: Failed to extract details: {e}")
