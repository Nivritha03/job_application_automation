...
class FounditJobParser(BaseJobParser):
    def extract_details(self, job: Job) -> Job:
        logger.info(f"FounditJobParser: Extracting details for {job.url}")
        try:
            try:
                self.page.goto(job.url, wait_until="domcontentloaded", timeout=15000)
            except Exception as e:
                logger.warning(f"FounditJobParser: Page goto warning: {e}")
                
            try:
                self.page.wait_for_selector("h1, div.job-description, .job-details", timeout=15000)
            except Exception as wait_err:
                logger.warning(f"FounditJobParser: Selector wait timeout: {wait_err}")
                
            time.sleep(1)
            
            # Check login redirects
            if "login" in self.page.url or "signin" in self.page.url:
                logger.error("FounditJobParser: Authentication required.")
            ...
            # Description
            desc_loc = self.page.locator("div.job-description, .job-details, .jd-desc").first
            if desc_loc.count() > 0:
            ...
            logger.error(f"FounditJobParser: Failed to extract details: {e}")
